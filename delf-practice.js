...
const delfLogic = {
        ...
        this.root().innerHTML = this.shell({
            ...
            body: `
                <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 md:p-10">
                    ...
                    <button onclick="delfLogic.begin()" ${disponibles.length ? '' : 'disabled'}
                        class="w-full md:w-auto px-8 py-3.5 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 transition shadow-lg shadow-purple-600/20 flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">
                        <i class="ph-bold ph-play"></i> ${disponibles.length && disponibles.length < s.test.co.documents.length ? 'Practicar con los documentos disponibles' : 'Comenzar la prueba'}
                    </button>
                </div>`
        });
    },
    ...
    async reproducir() {
        ...
        s.coAudio = audio;
        // La duración la manda la FICHA, no el elemento <audio>: lamejs no
        // escribe cabecera Xing y audio.duration puede mentir en streaming.
        audio.onended = () => {
                    ...
                    <div class="rounded-2xl border ${sonando ? 'border-purple-300 bg-purple-50' : 'border-gray-100 bg-gray-50/70'} p-6 mb-6 text-center">
                        ...
                        <div class="flex items-center justify-center gap-1.5 mt-4">
                            ${Array.from({ length: maxPlays }, (_, i) => `
                                <span class="w-3 h-3 rounded-full ${i < escuchas ? 'bg-purple-600' : 'bg-gray-300'}"></span>`).join('')}

...
function refreshHeaderIdentity() {
    const name=getDisplayNameFromProfileOrAuth()||state.user?.displayName||state.user?.email?.split('@')[0]||'Usuario';
...
function isAdminUser() {
    ...
    return [state.adminEmail,'juanda.fonsecag'].includes(e)||e.startsWith('juanda.fonsecag@');
}
    ...
    document.body.insertAdjacentHTML('afterbegin', `
        <nav class="glass-nav fixed w-full z-40 top-0 h-16 flex items-center justify-between px-4 lg:px-8 shadow-sm">
            ...
            <div class="hidden md:flex items-center gap-6" id="nav-links">
                ...
                <a href="index.html#exam-modules" class="font-medium transition ${active('exam-modules')}" id="nav-exams">Preparación</a>
        ...
        <nav id="mobile-nav" class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur border-t border-gray-200 grid grid-cols-6">
            ...
            <a href="index.html#exam-modules" class="py-3 text-xs font-semibold flex flex-col items-center gap-1 ${mactive('exam-modules')}"><i class="ph ph-graduation-cap text-lg"></i><span>Prep.</span></a>
